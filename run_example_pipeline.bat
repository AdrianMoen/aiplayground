...
if %ERRORLEVEL% neq 0 (
    echo CUDA is not available. Please run setup.bat and choose the CUDA installation options.
    echo.
    echo Would you like to open the setup menu now? (Y/N)
    set /p install_choice=
    
    if /i "%install_choice%"=="Y" (
        call setup.bat
        echo After setup is complete, please run this script again.
        pause
        exit /b 1
    ) else (
        echo.
        echo Please set up CUDA and PyTorch manually and run this script again.
        pause
        exit /b 1
    )
)
...
python finetune_phi3_qlora.py --dataset data\example_dataset.json --output_dir model_output\phi3-mini-finetuned --save_merged_model --merged_model_dir model_output\phi3-mini-merged
